...
训练：
  python -m scenicmind.flowstack.cli train --data path/to/training_data.xlsx --artifact-dir artifacts/flowstack/current
预测：
  python -m scenicmind.flowstack.cli predict --model-dir artifacts/flowstack/current/model \
      --features future_features.csv --output outputs/flowstack/predictions.csv
特征重要性（Agent 接口）：
  python -m scenicmind.flowstack.cli importance --model-dir artifacts/flowstack/current/model \
...
from scenicmind.flowstack.service import ImportanceService, PredictionService
