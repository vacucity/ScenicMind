...
const CAPTIONS = [
  { from: 90, duration: 40, text: '真实客流 · 预测趋势 · 一屏掌握' },
  // (the hero-card hover annotation lives inside SceneOpen as a 3D floating note)
  { from: 318, duration: 44, text: '上传一次 · 自动进入分析链路' },
  { from: 395, duration: 55, text: '7 天 · 14 天 · 30 天灵活预测' },
  { from: 477, duration: 68, text: '每一天，都有清晰的客流判断' },
  { from: 633, duration: 72, text: '8 大指标模块自动计算' },
  { from: 789, duration: 78, text: '从预测结果，继续追问经营原因' },
] as const;
    ...
    <AbsoluteFill style={{ backgroundColor: '#f3f0e8' }}>
      ...
      <Sequence from={AIFL_SHOTS.card1.from} durationInFrames={AIFL_SHOTS.card1.duration}>
        <PaperTitleCard
          ...
          words={[{ text: '上传一次，' }, { text: '未来客流', accent: true }, { text: '提前看见。' }]}
        />
      </Sequence>
      ...
      <Sequence from={AIFL_SHOTS.card2.from} durationInFrames={AIFL_SHOTS.card2.duration}>
        <PaperTitleCard
          ...
          words={[{ text: '指标蓝图，' }, { text: '自动计算', accent: true }, { text: '经营信号。' }]}
          sub="个核心模块"
          subDigits="8"
        />
      </Sequence>
      ...
      <Sequence from={AIFL_SHOTS.cardWbr.from} durationInFrames={AIFL_SHOTS.cardWbr.duration}>
        <PaperTitleCard
          ...
          words={[{ text: '让' }, { text: 'AI', accent: true }, { text: '解释每一次预测。' }]}
        />
      </Sequence>
      ...
      <Sequence from={AIFL_SHOTS.card3.from} durationInFrames={AIFL_SHOTS.card3.duration}>
        <PaperTitleCard
          ...
          words={[{ text: '景区决策，' }, { text: '快一步', accent: true }, { text: '抵达。' }]}
        />
      </Sequence>
