...
function CompactAccuracy({ mape, mae }: { mape: number | null; mae: number | null }) {
  ...
  return (
    <div className="indicator-block" id="module-accuracy">
...
/** 数字滚动动画（RAF + easeOutCubic，从上一值滚动到目标值） */
function AnimatedNumber({ value, decimals = 0 }: { value: number | null | undefined; decimals?: number }) {
...
function focusModule(id: string) {
  ...
  el.classList.remove("module-flash");
  void el.offsetWidth; // 重触发动画
  el.classList.add("module-flash");
  window.setTimeout(() => el.classList.remove("module-flash"), 1800);
}

/** 顶部 KPI 磁贴行（Power BI 风格：大号数字 + 涨跌箭头 + 点击定位） */
function KpiStrip({ indicators, mape }: { indicators: Record<string, any>; mape: number | null }) {
  ...
  const tiles = [
    { label: "最新客流", value: vt?.latest ?? null, unit: "人", tone: "neutral" as const, target: "module-trend", delta: vt?.momChange ?? null, deltaLabel: "环比" },
    { label: "7日均值", value: vt?.rollMean7 ?? null, unit: "人", tone: "neutral" as const, target: "module-trend", delta: vt?.yoyChange ?? null, deltaLabel: "同比" },
    { label: "载客率", value: cap?.latestLoadRate ?? cap?.avgLoadRate ?? null, unit: "%", decimals: 1, tone: (cap?.latestLoadRate ?? 0) >= 90 ? "up" as const : "neutral" as const, target: "module-capacity" },
    { label: "售罄天数", value: cap?.soldOutDays ?? null, unit: "天", tone: "neutral" as const, target: "module-capacity" },
    { label: "预测准确率", value: accuracy, unit: "%", decimals: 1, tone: accuracy != null && accuracy >= 90 ? "up" as const : "neutral" as const, target: "module-accuracy" },
    { label: "节假日提升", value: hol?.holidayLift ?? null, unit: "%", decimals: 1, tone: (hol?.holidayLift ?? 0) >= 0 ? "up" as const : "down" as const, target: "module-holiday", delta: hol?.holidayLift ?? null, deltaLabel: "提升" },
    { label: "雨天影响", value: wx?.rainImpactRate ?? null, unit: "%", decimals: 1, tone: (wx?.rainImpactRate ?? 0) <= 0 ? "down" as const : "up" as const, target: "module-weather" },
    { label: "高铁提升", value: tr?.hsrLiftRate ?? null, unit: "%", decimals: 1, tone: (tr?.hsrLiftRate ?? 0) >= 0 ? "up" as const : "down" as const, target: "module-transport" },
  ];

  return (
    <div className="kpi-strip" role="list" aria-label="核心经营指标">
      {tiles.map(tile => (
        <button key={tile.label} type="button" role="listitem" className={`kpi-tile kpi-tone-${tile.tone}`} onClick={() => tile.target && focusModule(tile.target)} title={`定位到「${tile.label}」模块`}>
          <span className="kpi-tile-label">{tile.label}</span>
          <span className="kpi-tile-value">
            <AnimatedNumber value={tile.value} decimals={tile.decimals ?? 0} />
            {tile.value != null && <small>{tile.unit}</small>}
          </span>
          {tile.delta != null && (
            <span className={`kpi-tile-delta ${tile.delta >= 0 ? "is-up" : "is-down"}`}>
              {tile.deltaLabel ? `${tile.deltaLabel} ` : ""}{tile.delta >= 0 ? "▲" : "▼"} {Math.abs(tile.delta).toFixed(1)}%
            </span>
          )}
        </button>
      ))}
  ...
  return (
    <section className="card indicator-card" id="indicators">
      <div className="section-heading">
        <div><h2>指标蓝图</h2><p>点击顶部 KPI 定位对应模块 · 8 大模块独立可视化</p></div>
      </div>

      <KpiStrip indicators={indicators} mape={mape} />

      <div className="indicator-grid">
        {/* 模块 1：客流趋势 - 30日柱状趋势 + KPI 行 */}
        {vt && <div className="indicator-block indicator-block-hero" id="module-trend">
          ...
          <MiniBarChart data={vt.recent30 || []} color="#316b51" height={170} label="近30日客流" />
        </div>}

        {/* 模块 2：承载与售罄 - 仪表盘 + 售罄/限流堆叠柱 */}
        {cap && <div className="indicator-block" id="module-capacity">
        ...
        {hol && <div className="indicator-block indicator-block-wide" id="module-holiday">
        ...
        {/* 模块 4：天气影响 - 降水对比柱 + 温度计 */}
        {wx && <div className="indicator-block" id="module-weather">
        ...
        {tr && <div className="indicator-block" id="module-transport">
        ...
        {att && <div className="indicator-block" id="module-attention">
        ...
        {/* 模块 7：数据质量 - 堆叠分布 + 进度条 */}
        {dq && <div className="indicator-block" id="module-quality">
...
function FeatureContribution({ importance, onAskAgent }: { importance: ImportancePayload | null; onAskAgent?: (question: string) => void }) {
  ...
  return (
    <section className="card contribution-card" id="contribution">
      ...
      {importance?.degraded && (
        <p className="contribution-degraded">⚠️ {importance.degraded_reason || "当前为降级模型，特征重要性仅反映历史客流滞后项，其余维度待模型恢复后呈现。"}</p>
      )}
...
/** 最新日预测达成率：最新真实日的真实客流 ÷ 当日模型预测值，衡量模型对最新一天的预测达成程度 */
function LatestDayAchievement({ result }: { result: AnalysisResult }) {
  const latest = result.latestActual;
  const predicted = latest.predictedVisitors;
  const ratio = latest.visitors > 0 && predicted != null && predicted > 0
    ? Math.max(0, Math.min(100, (latest.visitors / predicted) * 100))
    : null;
  const radius = 52;
  const circumference = 2 * Math.PI * radius;
  ...
  return (
    <section className="card admission-card">
      <div className="section-heading compact"><div><h2>最新日预测达成率</h2><p>最新真实日客流与当日模型预测值的比值</p></div></div>
      <div className="ring-wrap">
        <svg className="accuracy-ring" viewBox="0 0 130 130" role="img" aria-label={`最新日预测达成率 ${ratio == null ? "未知" : ratio.toFixed(1) + "%"}`}>
          <circle cx="65" cy="65" r={radius} className="ring-track"/>
          <circle cx="65" cy="65" r={radius} className="ring-progress" style={{ strokeDasharray: `${circumference}`, strokeDashoffset: offset }}/>
        ...
        <dl className="ring-stats">
          <div><dt>最新真实</dt><dd>{latest.visitors.toLocaleString()} 人</dd></div>
          <div><dt>模型预测</dt><dd>{predicted != null ? predicted.toLocaleString() : "—"} 人</dd></div>
          <div><dt>达成率</dt><dd>{ratio == null ? "—" : `${ratio.toFixed(1)}%`}</dd></div>
        </dl>
      </div>
    </section>
  );
          ...
          <div className="dashboard-content">
            ...
            <div className="dashboard-grid">
              ...
              <aside className="insight-column" aria-label="分析信息">
                <LatestDayAchievement result={result}/>
